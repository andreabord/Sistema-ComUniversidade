...
<div class="modal modal-large" id="modal-visualizar-{{$idContato}}">
    ...
    <div id="title-date-offer" style="margin-bottom:1rem;">
        <h4 title="{{$oferta->titulo}}">{{$oferta->titulo}}</h4>
    ...
    @elseif ($respostaMensagem == null && $oferta->tipo == 'CONHECIMENTO')
        <!-- Responder para ofertas de tipo "CONHECIMENTO" -->
        <form id="form-contato-{{$idContato}}" action="{{ route('contato_recebido_store_professor', [$idContato]) }}" method="POST" onsubmit="return validarEnviarFormularioConhecimento({{$idContato}})">
            @csrf
            <div class="receiver-message">
                <p class="p-info" style="margin-bottom:1rem;">Escreva sua resposta</p>
                <textarea class="input-text" name="resposta-contato" id="mensagem-contato-{{$idContato}}" placeholder="Existe alguém interessado em sua oferta, responda aqui (*Obrigatório)" oninput="habilitarBotoesConhecimento({{$idContato}})"></textarea>
            </div>

            <div class="buttons-container">
                <button class="button-b-primary" id="botao-interessado-{{$idContato}}" name="tipo_mensagem" type="submit" disabled>
                    Interesse
                </button>
                <button class="button-b-primary" id="botao-sem-disponibilidade-{{$idContato}}" name="tipo_mensagem" type="submit" disabled>
                    Sem disponibilidade
                </button>                    
    ...
    @elseif ($respostaMensagem == null && $oferta->tipo == 'ACAO')
        <!-- Responder para ofertas de tipo "ACAO" -->
        <form id="form-contato-{{$idContato}}" action="{{ route('contato_recebido_store_professor', [$idContato]) }}" method="POST" onsubmit="return validarEnviarFormularioAcao({{$idContato}})">
            ...
            <div class="receiver-message">
                <p class="p-info" style="margin-bottom:1rem;">Escreva sua resposta</p>
                <textarea class="input-text" name="resposta-contato" id="mensagem-contato-{{$idContato}}" placeholder="Existe alguém interessado em sua oferta, responda aqui (*Obrigatório)" oninput="habilitarBotoesAcao({{$idContato}})" style="flex: 1;"></textarea>
            </div>

            <div class="buttons-container">
                <button class="button-b-primary" id="botao-contato-respondido-{{$idContato}}" name="tipo_mensagem" type="submit" disabled>
                    Responder
                </button>
                <button type="button" class="button-b-secondary" onclick="closeModalVisualizarContatoRecebido({{$idContato}})">Fechar</button>
...
<div class="modal-out" id="clicar-fora-modal-confirmar-contato-respondido-{{$idContato}}" onclick="closeModalConfirmaContatoRespondido({{$idContato}})"></div>
<div class="modal modal-small" id="modal-confirmar-contato-respondido-{{$idContato}}">
    <p>Confirma o envio da mensagem?</p>
    <div class="buttons-container-confirmation">
        <button class="button-b-primary" id="botao-confirma-envio-resposta-{{$idContato}}">Confirmar</button>
        <button class="button-b-secondary" onclick="closeModalConfirmaContatoRespondido({{$idContato}})">Cancelar</button>
    </div>
</div>
</body>
</html>